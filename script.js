...
people.map(person => {
  firstLast.push(person.split(', '))
})



firstLast.forEach(fullNames => {
  let temp = fullNames[0]
...
inventors.reduce(function (acc, inventor) {
  // This was a tough one. But turns out can be solve with a single line
  return totalYearsLived = acc + (inventor.passed - inventor.year)
